...
export function App() {
  ...
  const [status, setStatus] = useState({ error: null, pending: true });
  ...
  // Held as one object but compared field by field, so a preview that reports
  // "still fine, still idle" does not re-render the app.
  const handleStatus = useCallback((next) => {
    setStatus((prev) =>
      prev.error === next.error && prev.pending === next.pending ? prev : next,
    );
  }, []);
    ...
    <>
      ...
      <div id="container" className="split-container">
        ...
        >
          ...
          {status.error && <div className="mdy-error-bar">{status.error.message}</div>}
          <div id="preview-wrapper">
            <div
              id="output"
              className={`content markdown-body${status.error ? ' is-stale' : ''}`}
            >
