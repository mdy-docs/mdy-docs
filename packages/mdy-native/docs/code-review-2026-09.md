...
`open_dir_inner` parses them once as YAML ([engine_walk.c:862](../src/engine_walk.c#L862))
and the mapping travels beside the document in a new `ident_data`
([engine_internal.h:297](../src/engine_internal.h#L297)), merged in `mdy_engine_open` after the
document's own fields and before `path`
([engine.c:647](../src/engine.c#L647)) — which is where mdy-docs puts a
...
`data_file_checks` ([test/engine.c:260](../test/engine.c#L260)), which fails
...
itself accepts it ([yaml.c:1096–1099](../src/parse/yaml.c#L1096-L1099)).
...
([engine_walk.c:943](../src/engine_walk.c#L943)) instead of counting beforehand and
re-deriving afterwards. Identity is collected per FILE while the walk runs
(`WalkedFile`, [engine_walk.c:647–653](../src/engine_walk.c#L647-L653)) and expanded
to one entry per document once the count is known
([engine_walk.c:970](../src/engine_walk.c#L970)). The two computations that had to
...
([engine.c:469](../src/engine.c#L469)), so both ways in run the same code.

Regression test: `blank_file_checks`
([test/engine.c:371](../test/engine.c#L371)), which fails on the old code in
...
jumps to `done:` ([engine.c:3547](../src/engine.c#L3547)) like every other
...
`FAIL` ([engine.c:3154](../src/engine.c#L3154)) and gives back the depth and
the `current` it had already taken; the cycle guard moved ABOVE everything
`done:` restores ([engine.c:3118](../src/engine.c#L3118)), which is what
...
([test/engine.c:458](../test/engine.c#L458)). It reproduces both shapes on a
...
([engine.c:859](../src/engine.c#L859)) — open addressing on the 24 hex
characters, built the first time a query asks and freed with the set, so it is
exactly as valid as `ids` is. `index_of_id`
([engine.c:885](../src/engine.c#L885)) is a lookup rather than a scan.

The ordering pass reads each hit's `_id` ONCE
([engine.c:967](../src/engine.c#L967)) and sorts the hits by the index it
...
([test/engine.c:546](../test/engine.c#L546)). It PASSES on the old engine too,
...
([engine.c:445](../src/engine.c#L445)), so a set's data dies with the set.
...
Regression test: `reopen_checks` ([test/engine.c:632](../test/engine.c#L632)).
...
([engine.c:3057](../src/engine.c#L3057)). `canonical_hash` grew one parameter
for it ([engine.c:2938](../src/engine.c#L2938)) — a single key left out, of
...
record ([engine.c:3044](../src/engine.c#L3044)). mdy-docs folds in its own
...
Regression test: `memo_key_checks` ([test/engine.c:779](../test/engine.c#L779)).
...
- `parse_flow` ([yaml.c:584](../src/parse/yaml.c#L584)) — `MDY_YAML_MAX_DEPTH`
...
- `js_to_tree` ([engine_value.c:233](../src/engine_value.c#L233)) — a document can build a
  ...
  deep.
- `canonical_hash` ([engine.c:3046](../src/engine.c#L3046)) — the same document
...
`test/engine.c` ([719](../test/engine.c#L719)) — two thousand rather than sixty
...
**Fixed.** `put_quoted` ([engine_walk.c:151](../src/engine_walk.c#L151)) writes an
...
([engine_walk.c:746](../src/engine_walk.c#L746)) into a buffer that grows, which
...
Regression test: `odd_name_checks` ([test/engine.c:2008](../test/engine.c#L2008)),
...
([engine_walk.c:744–744](../src/engine_walk.c#L744-L744), [882](../src/engine_walk.c#L882)).
...
([yaml.c:732](../src/parse/yaml.c#L732)), checked where `parse_value_from`
...
([yaml.c:884](../src/parse/yaml.c#L884)) — a comment is not one of the
...
([266](../test/yaml.c#L266)) cover both, the line a MULTI-line quoted scalar
...
([cli.c:1800](../src/cli.c#L1800)): 500 returns the messages to the broker,
...
([cli.c:1675](../src/cli.c#L1675)); they stay queued for the drain after the
next good build. And `mdy_engine_page_index` and `mdy_engine_document_path`
tolerate a NULL engine ([engine.c:3448](../src/engine.c#L3448)), which is the
...
([Makefile:296](../Makefile#L296)) and a CI step: it spawns the binary on a
...
([images.c:181](../src/images.c#L181)) — the bounds check and the per-entry
...
([test/engine.c:2081](../test/engine.c#L2081)) — the wrapping offset in both
...
there ([test/engine.c:3382](../test/engine.c#L3382)) — forty times over, and
...
([engine.c:2690](../src/engine.c#L2690)). mdy-docs can write the number in as a
...
([engine.c:3349](../src/engine.c#L3349)) and is read at object construction, so
...
([engine.c:3120](../src/engine.c#L3120)), which is the same argument that
...
- **`path` first** ([engine_walk.c:791](../src/engine_walk.c#L791)), because
...
- **`$.data` and `res.data` carry no store id**
  ([engine.c:1084](../src/engine.c#L1084)) — the rule `wrap()`'s `__answer`
...
with B29 ([engine.c:3132](../src/engine.c#L3132)), and `count_checks` in
...
([cli.c:799](../src/cli.c#L799)), `rc` carries the answer to it
([734](../src/cli.c#L734)), and the cleanup is written once — the third copy
in this file, after `mdy dev`'s ([1803](../src/cli.c#L1803)) and document
...
([cli.c:1526](../src/cli.c#L1526)). What the list is for is not sending the
...
#### B44 — a list item holding a block was not padded (Low) — FIXED

Found while checking the table shapes for §4's foster-parenting fix, and
separate from it: it has nothing to do with tables.
...
```
before  li: [ blockquote ]
after   li: [ text("\n"), blockquote, text("\n") ]   = node
```

**The rule, which this finding first stated wrongly.** It was filed as a
looseness bug: "`loose` is the list's `spread` ORed with whether the item has a
single child that is not a paragraph". Reading mdast-util-to-hast's `listItem`
rather than inferring it from the outputs, that is not what happens. `loose`
there is the LIST's and nothing is ORed into it; the padding is decided per
child:

> a newline before each child unless it is the first AND the list is tight AND
> that child is a paragraph; a trailing newline unless the last child is a
> paragraph in a tight list; and a paragraph child in a tight list is unwrapped
> into its own children.

Every other block parent in that file takes `wrap(nodes, loose)` — pad only
when loose — and a list item is the one that does not.

**Which makes the fix one line.** In a tight list md4c never reports the item's
paragraph at all and hands the inline content straight up, so the "unwrap the
paragraph, skip its padding" half of the rule is already true here by the shape
of the callbacks. What was missing is the other half: a child that IS a block
gets its newlines whether the item is tight or not. So a list item's frame is
always wrapped ([markdown.c:502](../src/parse/markdown.c#L502)), and tight
versus loose stays where md4c already decided it. That leaves the `list_tight`
stack the old rule consulted written and never read, so it goes with it — a
field recording tightness that nothing asks about would tell the next reader
this file still decides looseness here.

**And a second one, which the first exposed.** `- hi\n  > q` then gave
`text("hi")`, `text("\n")`, `blockquote` where node gives `text("hi\n")`,
`blockquote` — the same bytes in two nodes instead of one. A hast tree has no
two adjacent text nodes: nothing upstream produces them and the serialiser
...
**Measured tree-against-tree over three sets of shapes**, before and after:

```
fifteen list shapes         8/15  ->  15/15
eleven table shapes (§4)   10/11  ->  11/11     the twelfth shape, a table alone in an item
fifteen wider shapes        9/15  ->  12/15
```

Three of the wider set fixed themselves on the way — a nested tight list, a
quote with a list under it, a setext heading in an item — and were never
diagnosed separately, because they are this rule. Of the three still differing,
**two are the harness rather than the engine**: `build/mdcat` is the bare front
end, and the `.md` pipeline runs `rehypeRaw` after it, so a document whose item
holds raw HTML is re-parsed before anyone sees the tree. Both are identical
through a real build. The third is real, new, and is **B45**.

`fixture-awkward` carries a `.md` with a quote, a fence, a heading, a table and
a text-then-block item, so `check-sites`, `check-golden` and the wasm golden
...
([test/engine.c:545](../test/engine.c#L545)) is five assertions against what
...
([test/engine.c:779](../test/engine.c#L779)) is five assertions, the last of
...
[test/engine.c:779](../test/engine.c#L779) pins all five.
...
([engine.c:3547](../src/engine.c#L3547),
[3498](../src/engine.c#L3498), [3266](../src/engine.c#L3266)). All three carry
...
([test/engine.c:707](../test/engine.c#L707)); reverting the patch fails the
...
`caret_bracket_checks` ([test/engine.c:707](../test/engine.c#L707)) covers it
...
Filed after B35, where two unchecked allocations turned up in a file B24 had
swept. The reason they survived was that `check-alloc` runs `mdy build`, and
they were on the dev server's path. Extending the sweep to the other two
commands found **fourteen more**, three of which segfault.

| | swept | found |
| --- | --- | --- |
| `document` — `mdy <path> -o` | 922–395 per fixture | 4 (2 segfaults) |
| `dev` — the server | 1,885 | 10 (1 segfault) |

**Four of them are sites B24 named and parked** — `outputs_put`
([cli.c:321](../src/cli.c#L321)), `seen_before`, `add` in watch.c
([watch.c:34](../src/watch.c#L34)) and `broker_request`
([broker.c:50](../src/broker.c#L50)). They were parked because they are `void`
helpers with no error channel, which is exactly what `xalloc.h` was later
built for; what kept them unfixed after that was that nothing reached them.
`mdy build` writes each page as it is produced and only the dev server holds
them in an `Outputs`, so the sweep never once entered `outputs_put`.

**Document mode's four.** `doc_source` ([cli.c:879](../src/cli.c#L879)) is the
`[read]` logger, and document mode has its own — `build`'s is a different
function, which is why the existing sweep never touched it; it grows on the
FIRST call, since cap and count both start at zero, and wrote through the
result. The trailing-newline append ([cli.c:1382](../src/cli.c#L1382))
reallocs and writes on the next line. Both segfault.

The other two are in shared code and matter more:

- `mdy_data_extract` returning NULL was taken as "this body has no ```data
  fences" ([engine.c:558](../src/engine.c#L558)). A body with none comes back
  as an empty set; NULL means it could not allocate. So a document lost every
  fence it had — its data and its tags simply were not there — on a build that
  reported success. `examples/hashtags.mdy` lost a tag line at fifteen
  different ordinals.
- `oid_map_build` failing made `index_of_id` answer −1
  ([engine.c:875](../src/engine.c#L875)), which is a real answer meaning "no
  document of this set has that id". Every hit was then dropped and `$.find`
  quietly returned fewer documents than matched. It does not fail now.

**The server's invariant had to be restated**, and it is worth being exact
about what it now asserts. "The same site, or a reported failure" fits a
process that exits; a server offers neither. What it does offer is:

> it serves the same site, or it says it could not — and *saying so* includes
> refusing the request. What it must never do is answer **200 with bytes that
> are not the site's**.

That keeps the property that matters and lets three real behaviours through: a
dropped connection under memory pressure (the client is told, in the only
place an HTTP client listens), a first build that fails while the server stays
up — which is deliberate, B10 — and a 404. A third outcome exists here and
nowhere else and is counted on its own: the server neither dies nor answers.
Nothing produced it, but a sweep that could not tell a wedge from a refusal
would be worth much less.

**Two harness mistakes, both of which would have reported phantom bugs.** The
reference for `dev` was a `mdy build` at first — but the server injects the
live-reload script into every HTML page, so every HTML file compared unequal.
The reference is an unarmed *dev* run now; the build only supplies the list of
paths. And `examples/document-set.mdy` prints document ids, which are new
every run: it reported 35 wrong ordinals including the first, which is the
tell. It is not a sweep fixture, and the Makefile says why.

**Chasing an ordinal by hand does not work here**, which cost an hour before
...
`mdy dev` also needed the shim to report on **SIGTERM** — it never exits, so
`atexit` never fires and there was no total to sweep to. The handler formats
the number by hand and uses `write()`, because those are the rules inside a
signal handler.

`mdy dead` is not swept. It needs a broker to do anything, its surface is one
HTTP round trip and a report, and `test/dev.test.js` already drives it against
...
([engine_value.c:460](../src/engine_value.c#L460)) is the store-to-guest side,
...
([380](../src/engine_value.c#L380)) is the guest-to-store side, so a query for
an infinity asks for null and matches nothing. And a guest's non-finite tree
property is left unset ([344](../src/engine_value.c#L344)), which is the same
...
([yaml.c:173](../src/parse/yaml.c#L173)) — the same call `core_float` twenty
...
the way out is enough ([markdown.c:753](../src/parse/markdown.c#L753)).
...
([markdown.c:298](../src/parse/markdown.c#L298)), which is what
...
`attr_entity_checks` ([test/engine.c:940](../test/engine.c#L940)), every
...
- **Arming from outside.** emscripten's `getenv` reads its own `ENV` object and
  ...
  ([allocfail.c:82](../src/allocfail.c#L82)) are exported from the module and
...
([check-alloc.mjs:165](../wasm/check-alloc.mjs#L165)).
...
([yaml.c:1126](../src/parse/yaml.c#L1126)). The marker and whatever trails it
...
([fsx.c:183](../src/fsx.c#L183)), `fsx_list`'s sort counts and splits on it
([fsx.c:276](../src/fsx.c#L276)), and all three readers became the same one
line — `for (const char *rel = listing; *rel; rel += strlen(rel) + 1)` — in
the engine's walk ([engine_walk.c:704](../src/engine_walk.c#L704)), `cli.c`'s static
copier ([637](../src/cli.c#L637)) and `watch.c`'s snapshot
([52](../src/watch.c#L52)). Each of them lost a `strchr`, a mutation of the
...
([test/engine.c:2008](../test/engine.c#L2008)), which now carries a newline
...
- **~~B13 — GC rooting that works by luck.~~ FIXED, and the luck is now
  measurable.** The rule
  ([engine_value.c:90–109](../src/engine_value.c#L90-L109)) is that a value
  ...
  **Why they survived, and why that is the real finding.** `MDY_GC_STRESS`
  ...
  not ([engine_value.c:80](../src/engine_value.c#L80)), which enforces the rule
  ...
  ([engine_value.c:150](../src/engine_value.c#L150)) roots the object and
...
- **~~B14 — `report()` uses `strftime("%l")`~~ FIXED, and it is worse than
  ...
  ([cli.c:1147](../src/cli.c#L1147)); the duplicate formatting is gone and
...
- **~~B15 — Dev server leaks a refused publish's response.~~ FIXED.** The
  ...
  ([cli.c:1573](../src/cli.c#L1573)). The zeroing is what lets the free be
  ...
  ([cli.c:2105](../src/cli.c#L2105)). The other four `http_request` callers are
...
- **~~B16 — No socket timeouts~~ FIXED.** Measured before: `mdy dead` against
  ...
  **connect.** There is no socket option for this; the portable way is to go
  ...
  `connect_timeout` ([http.c:131](../src/http.c#L131)) is written out longhand
  ...
  **send.** The same deadline-on-the-whole-write as
  ...
  ([http.c:202](../src/http.c#L202)).

  Budgets: 5s to connect ([http.c:72](../src/http.c#L72)) and 15s for the
  ...
  **The IPv6 literal**, which was the other half. `parse_url` split host from
  ...
  are handled and stripped ([http.c:177](../src/http.c#L177)), since
  ...
  ([http.c:70](../src/http.c#L70)) and `realloc`'s result is checked — it went
...
- **~~B17 — Dev server exposure.~~ FIXED — four separate things, each with a
  ...
  **The bind.** It bound `0.0.0.0`, so every machine on the network could reach
  ...
  ([cli.c:2066](../src/cli.c#L2066)) opts back in and says so on stderr when it
  ...
  **The token.** Four `rand()` calls seeded `time(NULL) ^ &argc`: thirty-two
  ...
  ([httpd.c:144](../src/httpd.c#L144)), 19 bytes from `/dev/urandom` or
  ...
  ([cli.c:2082](../src/cli.c#L2082)), because a token that looks random and is
  ...
  **The request cap.** There was none: `recv` appended and the buffer doubled,
  ...
  cap at 16 MiB ([httpd.c:60](../src/httpd.c#L60)) and a 413 before the close.
  ...
  **The blocking write.** One thread serves everything, so a client that asks
  ...
  ([httpd.c:200](../src/httpd.c#L200)), and because the clock is
...
- **~~B18 — Watcher scan is O(n²)~~ FIXED.** It is a merge now
  ([watch.c:96](../src/watch.c#L96)). Both snapshots come from `fsx_list` in
...
- **~~B19 — `mdy build`/`dev`/`dead` accept anything as the positional~~
  ...
  the words all four commands use ([cli.c:434](../src/cli.c#L434)), together
...
- **~~B21 — Undefined behaviour on double→integer casts~~ FIXED — and it was
  ...
  The real sites were [ingest.c:35](../src/ingest.c#L35),
  [engine_value.c:380](../src/engine_value.c#L380),
  [bjval.c](../src/bjval.c#L127), [ast.c](../src/parse/ast.c#L258) and
  [html.c](../src/parse/html.c#L193).
...
- **~~B23 — markdown link attributes ignore entity substrings~~ FIXED.**
  ...
  ([markdown.c:394](../src/parse/markdown.c#L394)), resolving `MD_TEXT_ENTITY`
...
- **~~B24 — Unchecked allocations that dereference on failure~~ FIXED.** The
  ...
  **Propagated** — the channel existed and the caller was ignoring it:
  `fsx_read`'s NULL in the walk ([engine_walk.c:760](../src/engine_walk.c#L760)),
  `fsx_stat`'s return, which was dropped entirely — the document's record then
  claimed an empty file last written in 1970
  ([engine_walk.c:737](../src/engine_walk.c#L737)),
  `fsx_list`'s in `copy_static` ([cli.c:684](../src/cli.c#L684)) and in
  [fsx_list](../src/fsx.c#L287) itself, `mdy_to_html`'s in
  [fill_tokens](../src/engine_compose.c#L304), the identity copies
  ([engine_walk.c:1008](../src/engine_walk.c#L1008)), `rewrite_imports`'
  two unchecked `strdup`s ([engine_walk.c:547](../src/engine_walk.c#L547)),
  ...
  **Made infallible** — no channel, and the failure was silently different
  ...
  ([engine_walk.c:161](../src/engine_walk.c#L161)), `add_tag`
  ([engine_walk.c:306](../src/engine_walk.c#L306)), `scan_hashtags`
  ([engine_walk.c:317](../src/engine_walk.c#L317)), `cache_put`
  ([engine_walk.c:615](../src/engine_walk.c#L615)), the composition tokens
  ([engine_compose.c:95](../src/engine_compose.c#L95)), `tokenize_native`
  ([engine.c:58](../src/engine.c#L58)), the contents list and the text walk it
  uses ([collect_headings](../src/engine.c#L1699),
  [collect_text_into](../src/engine.c#L314)), `canonical_hash_deep`
  ([engine.c:3064](../src/engine.c#L3064)), `set_context_json`
  ([engine.c:720](../src/engine.c#L720)), the resize dedupe table
  ([engine.c:2252](../src/engine.c#L2252)) and `absolute`
  ([cli.c:262](../src/cli.c#L262)).

  **Three that needed a new channel.**

  `run_query_in` ([engine.c:910](../src/engine.c#L910)) is the one worth
  ...
  happened. `document_record` ([engine.c:1045](../src/engine.c#L1045)) did the
  ...
  **[MDY_YAML_OOM](../src/parse/mdyyaml.h#L87)**: one exact string, documented
  ...
  [oom()](../src/parse/yaml.c#L107) without the `line N:` every other message
...
- **~~B25 — `walk` treats every `opendir` failure as an empty directory~~
  ...
  anything else is an error ([fsx.c:216](../src/fsx.c#L216)), with the Windows
...
- **~~B26 — Local-bus and remote-bus disagree on an undeliverable subject~~
  ...
  ([cli.c:1696](../src/cli.c#L1696)), where both paths reach it: no page and
...
- **~~B27 — `wrap()` assembles the document's source with `snprintf("%s…")`~~
  ...
  pieces are `memcpy` now ([engine.c:2740](../src/engine.c#L2740)), which has
...
**Duplicated rather than unused.** Four of the eight were folded on the first
pass — the ones where the copies had drifted or the duplication was hiding
something. The other four were left because folding them means choosing where
a shared utility lives, and at the time nothing could hold one: `engine.c` was
about to be split, and three of the four straddled the parser/backend
boundary.

**All four are folded now, and the homes are these.** The split (§3) did not
supply them, and the reason it could not is worth stating, because it is what
made the question look harder than it was: `engine_internal.h` is private to
the engine and the parser cannot see it. But the traffic only ever needed to
go the OTHER way. The parser's *public* headers are already on the engine's
include path — that is how the engine reaches `mdy_parse` at all — so a
utility the two share belongs in one of those, and two of the four went there.

- **ASCII case-insensitive comparison** — `mdy_lower_ascii` and `mdy_ieq`
  ([mdytext.h:48](../src/parse/mdytext.h#L48), implemented at
  [unicode.c:141](../src/parse/unicode.c#L141)). `mdytext.h` already offers
  the engine `mdy_lower_full`, `mdy_trim` and the UTF-8 pair; ASCII folding is
  the same kind of thing and now sits beside them. Gone from `attrs.c`
  (`lower_ascii`), `cli.c` (`ieq`), and `engine_walk.c`, where `is_image_ext`
  and `ends_with_ci` are each two lines over `mdy_ieq` rather than their own
  loop. ASCII and not Unicode deliberately: every caller is comparing a file
  extension, an attribute name or a doctype, which the specifications define
  as ASCII-insensitive and where folding `İ` would be wrong.
- **Path normalisation** — `fsx_normalize`
  ([fsx.h:88](../src/fsx.h#L88), implemented at [fsx.c:48](../src/fsx.c#L48)),
  which is the home the first pass nominated. `resolve_path` and `absolute`
  are each four lines over it now.

  What did **not** fold is the joining, and that turned out to be the
  interesting half: `absolute` translates `\` to `/` and `resolve_path` must
  not. One is handling a path a person typed on Windows; the other an import
  specifier, where `a\b.mdy` is a legal POSIX filename. The difference reads
  like an oversight in two copies of one algorithm and is load-bearing, so the
  two keep their own entry points over one shared core.
- **The growable byte buffer** — **two** of them, one per library, and that is
  the answer rather than a failure to find one:

  | | where | fails? |
  | --- | --- | --- |
  | `mdy_buf` | [internal.h:47](../src/parse/internal.h#L47) | yes — carries `ok` |
  | `mdy_sbuf` | [xalloc.h:58](../src/xalloc.h#L58) | no |

  The parser's carries an `ok` flag because a parse has a caller to report to,
  and B24 made every one of those reports real. The engine's cannot fail,
  because since B24 the engine allocates through `xalloc.h` — so an `ok` there
  would be a field nothing could set. Merging them would mean one side
  carrying machinery it has no use for.

  Eight copies became two: `html.c`, `script.c`, `yaml.c`, `data.c` and
  `ast.c` onto the first; `fsx.c`, `cli.c` and `bjval.c` onto the second. The
  four parser copies differed only in the capacity of their first allocation —
  8192, 4096, 128, 1024, each sized to what that caller writes — which is a
  `seed` field now rather than four functions. `ast.c` keeps its `int`-
  returning convention behind a two-line adapter, because thirty-five call
  sites propagate it and changing them would be churn for its own sake.

  **It was not only tidying.** Two of the three engine-side copies grew with
  an **unchecked `realloc` and wrote through the result** — `cli.c`'s
  `--emit-js` buffer and `bjval.c`'s JSON serialiser. Both are B24's class and
  neither is reachable by any sweep: `--emit-js` is a flag the document sweep
  does not pass, and `bjv_to_json` runs only when a message is actually
  delivered, which the dev sweep does not do. They are B43's gap, found by
  reading rather than by running.
- **UTF-8 encoding** — folded onto `mdy_utf8_encode`, which `mdytext.h`
  already declared: `markdown.c`'s `utf8_of` is a one-line macro over it
  ([markdown.c:168](../src/parse/markdown.c#L168)) and `yaml.c`'s
  `put_codepoint` encodes into four bytes and puts them.

  **`in_ranges` stays duplicated**, and this one is not a missing home either.
  Its parameter type `UCDRange` comes from baru-re's `ucd.h`, which also
  defines the tables — `static const UCDRange ucd_bin_ASCII_ranges[] = {…}`
  and thirty more like it. Putting the declaration anywhere shared means
  including that header there, which copies every table into every translation
  unit that includes it. Nine lines of binary search, written twice, is the
  cheaper of the two.

`inline.c`'s pending-text buffer is not one of these and was never counted:
it is FIXED at four times the input, which is B20's fix, and the comment above
`push` says why that bound holds.


- ~~UTF-8↔UTF-16~~ **FOLDED, and it was a bug.** `to_utf16`/`from_utf16` in
  engine.c are wrappers over `mdy_to_utf16`/`mdy_from_utf16` now
  ([engine_value.c:40](../src/engine_value.c#L40)); they allocate, which is what every
...
- ~~The `tags:` YAML writer, twice~~ **FOLDED, and it was hiding a bug.** Both
  ...
  There is one writer now ([engine_walk.c:216](../src/engine_walk.c#L216)) over one
  escaper ([engine_walk.c:183](../src/engine_walk.c#L183)), which `put_quoted` from
  B8's fix also uses — so everything this file emits as YAML is escaped by the
  same code. Regression test: `tag_checks`
  ([test/engine.c:1985](../test/engine.c#L1985)), a quote, a backslash and a tab
...
- **`done_list[4096]`** ([cli.c:1702](../src/cli.c#L1702)) is the body of the
...
- **The import rewrite** ([engine_walk.c:564](../src/engine_walk.c#L564)) is
  ...
  **four times**, and `import_line` admits 1,023 characters of it — so a line
  ...
  ([test/engine.c:1591](../test/engine.c#L1591)), which is ASan's to catch:
...
([3487–3595](../src/engine.c#L3487-L3595), ~300 lines) manages seven GC roots
...
([686–1108](../src/engine_walk.c#L686-L1108)) builds the synthetic source that
...
**Process-global state.** The memo tables ([engine.c:3084](../src/engine.c#L3084))
and `mdy_engine_rotate_memo(void)`, the nisaba slot table (`nis.c`),
`lookup_import`'s `static char path[1024]` ([1310](../src/engine.c#L1310)),
...
half.** One object per source ([Makefile:516](../Makefile#L516)), and three
...
([Makefile:537](../Makefile#L537)): the sources come from four directories, two
...
([Makefile:65](../Makefile#L65)) — and only when a `check-` target is what was
...
([images.c:30](../src/images.c#L30)). A clean build of all three binaries is
...
`api_checks` ([test/engine.c:2399](../test/engine.c#L2399)) is 23 assertions
...
`httpd.c`, `http.c` and `watch.c` had no test of any kind; `mdy dev` had one.
That is less true than it was — `test/dev.test.js` is eleven tests now, and
B43's sweep drives the server, the watcher and the HTTP layer through 1,889
allocation failures apiece.

**The sixth `check-sites` fixture is built** — `fixture-awkward`
([Makefile:651](../Makefile#L651)), a site made of the inputs that WERE bugs:
an empty `.mdy` and a whitespace-only one (B2), a `.yaml` whose first line is
`---` and one carrying `+++` inside a block scalar (B1), a `.md` reached
through both `$.render` and `$.text` (B3), a seventy-column table (B20), a
`.tif` whose header points its IFD past the end of the file (B11), and the
whole set's order emitted as JSON, which is what B1 and B2 moved. Its
`main.mdy` names the finding each file stands on.

**It found a bug on its first run**, which is the argument for it in one line.
`wide.html` differed — and not by length: **1,892 bytes on both sides**, the
same newlines in a different place.

```
node   <table><thead><tr><th>…       preceded by eleven newlines
C      <table>\n<thead>\n<tr>\n…     with the newlines inside it
```

This engine reproduces remark-rehype's `wrap(nodes, loose)` exactly, which
pads a `<table>`, its row groups and its rows with newlines the way it pads a
`<ul>`. What it was missing is what happens NEXT in mdy-docs' `.md` pipeline:
...
caught it — and it needs a corpus outside the repo and never runs, which is
this section's own complaint. Every `.md` document with a table was affected.

Eleven table shapes now agree at the tree level. The twelfth — a table that is
the only child of a list item — differed too, for a reason that has nothing to
do with tables; that was **B44**, and it is fixed, so the twelfth agrees now
as well.

The fixture is in `GOLDEN_SITES` as well as `CHECK_SITES`, so it runs on every
platform CI builds for rather than only where node can. It was checked for
determinism first (`make check-determinism`), which is what that target is
for.

One thing is deliberately NOT in it: a file name carrying a `"`, which is B8.
Windows forbids a quote in a file name, so committing one would break the
checkout on a platform this project builds for. `test/engine.c` makes that
file at run time in a temp directory, which is the only way to test it in a
...
**~~Fragile initialisers.~~ FIXED.** `bjval.c` filled `bj_visitor`
positionally ([bjval.c:81](../src/bjval.c#L81)) while engine_value.c used
designated initialisers for the same struct. `on_num` appeared four times in
that list — for int, float, date and pointer — so a field reordered or
inserted in binjson's header would have kept compiling and bound the wrong
callback to the wrong type. binjson is a separate repository and has been
edited from here (B37), which is exactly the case this guards. It is filled by
name now; `memns.c`'s `bj_io` already was.

**~~Debug switches are read in hot paths.~~ FIXED, and measured first.**
`getenv("MDY_MEMO_DEBUG")` ran three times per render and
`getenv("MDY_LINEMAP_DEBUG")` once per produced line; they are read once each
now ([engine.c:21](../src/engine.c#L21)).

Measured before changing it, because the claim deserved a number: `docs-site`
built with **two thousand extra environment variables** was **1.9% slower**
than with a normal one, median of nine runs. So the cost is real and small —
this is not the hot path the heading implies. What it buys is that the cost is
provably nothing, and that a switch cannot be read differently in two places.
...
([engine.c:2554](../src/engine.c#L2554), [2569](../src/engine.c#L2569)) even
though `on_message` exists for exactly that. Pick two.

**Not done**, and it is the same trade as §3's long functions: unifying them
touches every error path in the engine, changes no behaviour, and is checked
by nothing but the parity suite. The one part that is a defect rather than a
preference — `fprintf` from inside a library that has a callback for it — is
two call sites and worth doing on its own the next time that file is open.
...
([engine.c:304](../src/engine.c#L304), [1126](../src/engine.c#L1126),
[3562](../src/engine.c#L3562)), the unused parameter above, and two from stb
...
**Every one of the fifty-one findings is fixed, and so is every structural
item below.** What follows is what those were and what became of them, plus
the one question the corpus leaves open.

**A caution about this file's citations.** `verify.py` says a citation
RESOLVES and that its display text matches its target. It cannot say the
target is still the right LINE, and a sample says several are not — B3's
pointed inside `memo_key`'s comment, and `broker.c:50`, `block.c:345` and
`raw.c:368` land on `r_write`, a `*/` and a `}`. Those three files have not
been touched since the citations were written, so the drift is older than any
of the work below; the citations into files that DID change were carried by
content and checked line by line (105 correct, 16 deliberately repointed).
Anything read out of this file that matters should be checked against the
code.
...
3. ~~**`struct mdy_engine`'s 56 fields**~~ **Done** — fifty-seven of them, by
   the time this was written, and they are seven named groups and four
   members now ([engine_internal.h:122](../src/engine_internal.h#L122)): `set`,
   `compose`, `knobs`, `cb`, `identity`, `graph`, `highlight`, with
   `session`, `vm` and `ctx` left flat because `e->js.vm` would have been 194
   edits to say nothing `e->vm` does not. Not done by sed — `->count`,
   `->root` and `->depth` are fields of three other structs here, so the
   fields were moved in the header and the COMPILER named every site.
   Two of the groups earn more than tidiness: `set` is one LIFETIME (it is
   exactly what `close_set` frees), and `compose` against `knobs` draws the
   line the flat list could not — what an engine has DONE against what it was
   ASKED for.
   **And the grouping was not free of findings.** Writing the callbacks as one
   struct turned six hand-written assignments into `child->cb = e->cb;`, which
   showed that the child engine inherited THREE of the five pairs: `on_source`
   was not among them, so an imported package's files were read without the
   `[read]` line node prints. Nothing compared the build log — `check-sites`
   built C with `--quiet`. It does not now, and that found a second one: node
   writes all pages, then all binaries, then `static/`, and this engine wrote
   a binary the moment a render produced it, which moved one line and shifted
   thirteen more over examples/blog. All six sites agree on their logs as well
   as their outputs.
4. ~~**The process-global memo**~~ **Done** — `mdy_session` owns it, and
   `mdy_engine_new` takes one. A rebuild is a new engine in the SAME session,
   which is the B6 property stated as a lifetime instead of relied on as a
   global; two sessions share nothing; `mdy_session_free` is the first time
   the memo's two tables have been freeable at all. §3 said this reaches "the
   wasm wrapper and mdy-live-preview-native" — measured, it reaches neither:
   both go through the CLI's `main()`.
5. ~~**`render_tree_out`'s length**~~ **Done** — 314 lines to 108, in eight
   named phases ([engine.c:3487](../src/engine.c#L3487)). §3's reason for not
   doing it was the seven GC roots, and that decided the shape: they are one
   `RenderRoots` struct, so every address the collector holds belongs to one
   frame. Then the part worth more than the split — a deliberately leaked root
   was caught by NOTHING: not the engine tests, not `MDY_GC_STRESS=1`, not the
   parity suite. So release walks the roots as an array with a
   `_Static_assert` tying the two together, and an eighth root that escapes it
   stops the build.
   ~~**and identity as text**~~ **Done** — `mdy_yaml` has a builder, so the
   walk hands over VALUES. There is no YAML source in between to escape or to
   mis-read, and 68 fewer allocations per build of fixture-awkward. The
   blocker §3 named — "there is no way to make an mdy_yaml mapping from C" —
   is what got built.
...
- `mdy_engine_on_message` is registered in **one** place in the whole CLI
  ([cli.c:1022](../src/cli.c#L1022)), and it is document mode. The messages in
  question come from the directory walk
  ([engine_walk.c:913](../src/engine_walk.c#L913)), which only runs under
...
([engine.c:2554](../src/engine.c#L2554)) are the same shape.
