...
int memo_debug(void)           { static int c = -1; return debug_flag("MDY_MEMO_DEBUG", &c); }
...
static mdy_doc *render_markdown_document(mdy_engine *e, size_t index, uint64_t mkey,
    ...
    if (mkey) memo_remember(e, mkey, out, mdy_xstrdup(text ? text : ""));
...
static void memo_keep(mdy_engine *e, uint64_t mkey, mdy_doc *out,
    ...
    memo_remember(e, mkey, out, text ? text : mdy_xstrdup(""));
}
