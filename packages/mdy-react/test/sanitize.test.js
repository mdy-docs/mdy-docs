...
import { renderToReact, mdySanitizeSchema } from '../src/index.js';
...
test('HTML containers survive sanitization', async () => {
...
test('scripts and event handlers are removed', async () => {
  ...
  const handler = await clean('<div onclick="alert(1)">click</div>\n');
...
test('React drops inline event handlers even unsanitized', async () => {
  assert.equal(await render('<div onclick="alert(1)">click</div>\n'), '<div>click</div>');
...
test('a raw script tag still runs unsanitized — the default is a choice', async () => {
  assert.match(await render('<script>alert(1)</script>\n'), /<script>alert\(1\)<\/script>/);
});
